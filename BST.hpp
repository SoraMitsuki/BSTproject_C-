/**
  ...
  * Date: 1/20/2017
  */
// this file is the basically same from BSTInt except this is header
// and method all combined together
...
std::pair<BSTIterator<Data>, bool> BST<Data>::insert(const Data& item) {
  ...
  if (!root) { //copied from BSTInt for comment please see BSTInt
  ...
  while (curr) {
    if (item < curr->data) {
      ...
      // changed return statement when insert fail retren false and the current position 
  ...
  ++isize;
  // changed return statement successful inserted, return true and the new node position
...
BSTIterator<Data> BST<Data>::find(const Data& item) const
{
  ...
  // HINT: Copy code from your BSTInt class and change the return value
  BSTNode<Data>* curr = root;
  while (curr) { //basically the samething from the BSTInt find method
    if (curr->data < item) {
      curr = curr->right;
    }
      ...
      // changed return statement: return the pointer to the current position
      return BSTIterator<Data>(curr);
    }
  }
  // when find fail return the pointer to the past position
 ...
 *  */
//this the recurssion helper method for Height 
// for comment please see BSTInt
template <typename Data>
int findHeight(BSTNode<Data>* n){
  if(n->left == NULL && n->right == NULL){
    ...
    int leftH = 0;
    int rightH = 0;
    leftH = findHeight(n->left);
    rightH = findHeight(n->right);
    if(leftH >= rightH){
      return 1 + leftH;
    }
    else{
...
int BST<Data>::height() const
{
  ...
  int height = 0;
  height = findHeight(root); //samething from BSTInt
  return height;
}
...
bool BST<Data>::empty() const
{
  ...
  if(!root){ //samething from BSTInt class
    return true;
  }
  else{
    return false;
...
{
  // TODO
  BSTNode<Data>* curr = root; //starting with the root of the BST
  while(curr->left != NULL){ //if the left of the BST does exist
    curr = curr->left; //return the left node
  }
  return curr; //if does not esxist return the root
}
...
void BST<Data>::deleteAll(BSTNode<Data>* n)
{
  // TODO
  // HINT: Copy code from your BSTInt class.
  if(n != NULL){ //samething from the BSTInt class
    deleteAll(n->left);
    deleteAll(n->right);
    delete n;
  }
